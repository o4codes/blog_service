...
class RssFeedDatabase:
    ...
    async def list(self, **query) -> List[RssFeed]:
        ...
        rss_feeds = [
            RssFeed(**rss_feed, id=rss_feed["_id"])
    ...
    async def get_by_provider_id(self, provider_id: str) -> List[RssFeed]:
        ...
        Returns:
            List[RssFeed]: list of rss feeds
        """
        rss_feeds = await self.collection.find({"provider_id": ObjectId(provider_id)}).to_list(None)
        rss_feeds = [
            RssFeed(**rss_feed, id=rss_feed["_id"])
            for rss_feed in rss_feeds
        ]
    ...
    async def create_many(self, rss_feeds: List[RssFeed]) -> List[RssFeed]:
        ...
        Returns:
            List[RssFeed]: list of rss feeds
        """
        result = await self.collection.insert_many([rss_feed.dict() for rss_feed in rss_feeds])
        rss_feeds = [
            await self.get_by_id(result_id)
            for result_id in result.inserted_ids
        ]
        return rss_feeds
    ...
    async def update(self, feed_id, rss_feed: RssFeed) -> RssFeed:
        ...
        Returns:
            RssFeed: rss feed
        """
        await self.collection.update_one({"_id": ObjectId(feed_id)}, {"$set": rss_feed.dict(exclude={"id"})})
    ...
    async def delete_many(self, provider_id: str) -> bool:
        ...
        """
        result = await self.collection.delete_many({"provider_id": ObjectId(provider_id)})
        return result.deleted_count > 0
