...
async def get_rss_provider_by_id(id: str, db=Depends(get_database)):
...
async def create_rss_provider(url: AnyUrl, db=Depends(get_database)):
...
async def update_rss_provider(id: str, url: AnyUrl, db=Depends(get_database)):
